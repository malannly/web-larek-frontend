...
1. Model карточки (CatalogModel):
    ...
    - храненит список карточек товаров
...
2. Model корзины (BasketModel):
    ...
    - храненит список товаров в корзине
    - управляет товарами (добавление, удаление)
...
3. Model покупателя (UserModel):
    ...
    - храненит данные пользователя (адрес, оплата, контакты и тд)
...
1. View карточки (ViewCatalog):
    - получaет список карточек из CatalogModel
    - создаео карточки товаров в HTML
    - добавляет кнопки "Добавить в корзину" с обработчиками

    Методы:
    - renderProducts(products: ICatalog[]): void - рисует карточки
    - AddToBasket(handler: (id: number) => void): void - обработчик клика "Добавить в корзину"

2. View корзины (ViewBasket):
    - показывает товары из BasketModel
    - добавляет обработчики, чтобы удалить товар (значок мусорки) и "Оформить заказ"

    Методы:
    - renderBasket(items: IBasket[]): void - рисует массив краточек
...
3. View покупателя (ViewUser):
    - показывает форму оплаты и доставки
    - обрабатывает ввод пользователя
    
    Методы:
    - renderUserForm(user: IUser): void - форма
    - SubmitUserData(handler: (userData: IUser) => void): void - обработка формы
...
- добавить карточку в корзину -> addToBasket() (вызываеться из BasketModel)
- удалить карточку из карзины -> removeFromBasket() (вызываеться из BasketModel)
- проверку на наличии товара в корзине -> catalogInBasket() (вызываеться из BasketModel)


2. BasketModel
Представляет собой:
- массив карточек (id - для отображения названия, цены), (общее число карточек) -> getCatalog()
